...
signal button_pressed(id, side)
signal button_released(id, side)
signal dpad_pressed(id, side)
signal dpad_released(id, side)
...
var _last_dpad_action = ""
remote func _on_input_direction_calculated(side, direction, intensity):
	var id = get_tree().get_rpc_sender_id()
	var _action_name = _last_dpad_action
	var _action_pressed = true
	if direction == Vector2(1, 0):
		_action_name = "ui_right"
		_last_dpad_action = "ui_right"
		_action_pressed = true
		print("going right")
	elif direction == Vector2(-1, 0):
		_action_name = "ui_left"
		_last_dpad_action = "ui_left"
		_action_pressed = true
		print("going left")
	elif direction == Vector2(0, 0):
		print("going anywhere :c")
		_action_pressed = false
	var event = InputEventAction.new()
	event.action = _action_name
	event.pressed = _action_pressed
	Input.parse_input_event(event)
