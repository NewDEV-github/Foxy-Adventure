...
class FMODPlayer:
	...
	func load_banks():
		for i in bank_list:
			...
			Fmod.loadbank(bank, FmodFlags.FMOD_LOAD_BANK_FLAGS.FMOD_STUDIO_LOAD_BANK_NORMAL)
	func create_sound_instances():
		Fmod.addListener(self)
		for i in stages_music_events:
			var inst = Fmod.createEventInstance(i, "events:/" + i)
	...
	func play_event_music(event_name, oneshot=false):
		Fmod.startEvent(music_instances[event_name])
		pass
	func play_event_sfx(event_name, oneshot=false):
		Fmod.startEvent(sfx_instances[event_name])
		pass
	func stop_event_sfx(event_name):
		Fmod.stopEvent(sfx_instances[event_name], FmodFlags.FMOD_STUDIO_STOP_MODES.FMOD_STUDIO_STOP_ALLOWFADEOUT)
	func stop_event_music(event_name):
		Fmod.stopEvent(music_instances[event_name], FmodFlags.FMOD_STUDIO_STOP_MODES.FMOD_STUDIO_STOP_ALLOWFADEOUT)
	func set_sfx_volume_db(volume):
		for i in sfx_instances:
			Fmod.setEventVolume(i, volume)
			pass
